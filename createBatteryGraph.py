...
def select_all_tasks(conn, y, m, d, h, t, p):
    ...
    for row in rows:
        ...
        Ibat          = row[3]
        capacity      = row[4]
        if debug2: print( "Ticks="+str(row_ticks)+" #"+str(batteryNumber)+" Vbat="+str(Vbat)+" Ibat="+str(Ibat)+" Capacity="+str(capacity)+"%" )

        # Calculate Watts discharging (<0) or charging (>0)
        # ignoring which battery it is! So average and x2
        usage = 2.0 * (Vbat/100.0) * (Ibat/100.0)
        if debug2: print( "Usage="+str(usage)+" W" )
        '''
...
def main():
    ...
    if (n == 3):
      ...
      yLabelGraph =" Charge(+)/Discharge(-) in Watts (W)"
      titleGraph  = "Battery Charging/SOC for day "+str(day)+"/"+str(month)+"/"+str(year)
    elif (n == 5):
      day   = int(sys.argv[3])
      hour  = int(sys.argv[4])
      #where = sys.argv[5]
      ...
      yLabelGraph =" Charge(+)/Discharge(-) in Watts (W)"
      titleGraph  = "Battery Charging/SOC for "+str(hour)+":00 hour on "+str(day)+"/"+str(month)+"/"+str(year)
    ...
    #             values( "+ timestamp +"," + battery +"," + Vbat + "," + Ibat + "," + capacity +")";
    # var topic="INSERT INTO battery_info " +msg;
