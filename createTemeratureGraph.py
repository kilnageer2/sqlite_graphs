...
def select_all_tasks(conn, y, m, d, h, t, p):
    ...
    for row in rows:
        ...
        battMv        = row[2]
        temp          = row[3] / 100.0
        if debug2: print( "Ticks="+str(row_ticks)+" room"+roomName+" battMv="+str(battMv)+" temp="+str(temp)+"C" )
        ...
        if ((row_ticks >= first_ticks) and (row_ticks <= tick_end)):
          ...
          #dt = datetime.datetime.fromtimestamp( row_ticks )
          #print(row_ticks, dt, usage)
          step_total = step_total + temp
          step_count = step_count + 1
          #print(row)
          #if (i == 5): break

          plot_tuples.append( (temp) )
'''
    if (p == "Hour"):
      # Four KWh we only calculate value at end of the hour :)
      #graph_total = graph_total + step_average
...
def main():
    ...
    if (n == 3):
      ...
      yLabelGraph =" Temperature (C)"
      titleGraph  = "Temperatures for month "+str(month)+"/"+str(year)
    elif (n == 4):
      day   = int(sys.argv[3])
      #where = sys.argv[4]
      period = "Day"
      ...
      yLabelGraph =" Temperature (C)"
      titleGraph  = "Temperatures for day "+str(day)+"/"+str(month)+"/"+str(year)
    elif (n == 5):
      day   = int(sys.argv[3])
      hour  = int(sys.argv[4])
      #where = sys.argv[5]
      ...
      yLabelGraph =" Temperature (C)"
      titleGraph  = "Temperatures for "+str(hour)+":00 hour on "+str(day)+"/"+str(month)+"/"+str(year)
    ...
    # msg="(timestamp, battery,voltage,current,capacity) 
    #             values( "+ timestamp +"," + battery +"," + Vbat + "," + Ibat + "," + capacity +")";
    # var topic="INSERT INTO battery_data " +msg;
