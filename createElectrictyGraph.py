...
def select_all_tasks(conn, y, m, d, h, t, p):
    ...
    for row in rows:
        ...
        if (match in ts1):
          ...
          if (p == "Day"):
              #print("match="+ts1)
              # We only want to plot the 10 minute averages
              # So let's average every 20 readings
              averageUsage = averageUsage + usage
              averagingCount = averagingCount + 1
              if (averagingCount >= averagingMax):
                  y_l.append( int(averageUsage / averagingMax) )
                  averageUsage = 0
                  ts2 = str(datetime.datetime.fromtimestamp(row_ticks, tz=None))
                  x.append( ts2 )
                  #print( "( "+ts2+", "+str(usage)+" ),")
          ...
          if (p == "Month"):
              ...
              if (same_month) :
                  ...
                  if (same_day) :
                      ...
                      totalUnitsForMonth = totalUnitsForMonth + last_usage
                      x.append( last_ts2 )
...
def main():
    ...
    if (n == 4):
      ...
      xAxisFormat = "%d"
      yAxisLabel = "Total daily KWh (units)"
      ...
      xAxisFormat = "%H:%M"
      yAxisLabel = "Wh ("+str(averagingMax)+" reading average)"
      yLabelGraph =" Units in Watts (W)"
    ...
    if (period == "Month"):
        ax.set(xlabel="Total units consumed = "+str(int(totalUnitsForMonth)), title=titleGraph)
    else:
        ax.set(title=titleGraph)
    ax.set_ylabel(yAxisLabel,color="blue",fontsize=14)
    ...
    formatter = matplotlib.dates.DateFormatter(xAxisFormat)
    #figure = plt.figure()
